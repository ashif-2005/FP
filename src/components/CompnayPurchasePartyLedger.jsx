...
const PurchaseCompanyPartyLedger = () => {
  ...
  const getCustomer = async () => {
    try {
      const res = await axios.post(
        `${url}/purchase-log/get?page=${page}&limit=${limit}`,
        {
          company: company,
        }
      );
  ...
  return token ? (
    <div className="page-container">
      <div className="page-header">
        <h1 className="page-title">{company}'s Purchase Ledger</h1>
      </div>
        ...
        <table className="data-table">
          ...
          <tbody>
            <tr>
              ...
              <td style={{ textAlign: "right", fontWeight: "bold" }}>{parseFloat(data.fp_balance).toFixed(2)}</td>
              <td></td>
              <td style={{ textAlign: "right", fontWeight: "bold" }}>{parseFloat(data.fp_balance).toFixed(2)}</td>
            </tr>
...
export default PurchaseCompanyPartyLedger;
