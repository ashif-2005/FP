...
const PurchaseInvoiceLedger = () => {
  ...
  const handleClick = (customer) => {
    navigate(`/purchase-invoice/${customer.name}`);
  };

  return token ? (
    <div className="page-container">
      <div className="page-header">
        <h1 className="page-title">Purchse Invoice Ledger</h1>
      </div>
            ...
            {customers.map((customer, index) => (
              <tr key={customer.id}>
                ...
                <td onClick={() => handleClick(customer)} style={{ textAlign: "right" }}>
                  {parseFloat(customer.purchase_balance).toFixed(2)}
                </td>
              </tr>
            ))}
          </tbody>
...
export default PurchaseInvoiceLedger;
